 ...
 *
 * <p>This is the only package in Rootstock allowed to import {@code com.revrobotics} (ArchUnit
 * rule 1). A REV-only team installs {@code dev.rootstock:rootstock-revlib} alongside REV's own
 ...
 *
 * <ul>
 *   <li>{@link org.rootstock.hardware.rev.SparkMotorIO} — the backend. Owns the device config
 *       object and derives every unit conversion in its {@code buildConfig} javadoc.
 *   <li>{@link org.rootstock.hardware.rev.RevGainSink} — the <b>only</b> code that converts
 *       canonical volts-per-SI gains into REV units.
 *   <li>{@link org.rootstock.hardware.rev.RevUtil} — the two, and only two, ways this library
 *       writes configuration to a SPARK: blocking-and-verified, and fire-and-forget.
 *   <li>{@link org.rootstock.hardware.rev.SparkAbsoluteEncoderIO} — a data-port absolute encoder,
 *       read in output-shaft rotations.
 *   <li>{@link org.rootstock.hardware.rev.RevBackend} — the {@code ServiceLoader} entry point.
 * </ul>
 *
