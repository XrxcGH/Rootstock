 ...
 *
 * <p>Instances are created by {@link PumpkinSim} from a {@link org.pumpkinlib.core.spi.MechanismGeometry}
 * and ticked by {@link PumpkinSim#tick()}. A team never implements this interface and never calls
 ...
 * exactly the class of unit lie the rest of PumpkinLib is built to prevent, so the shape of the design
...
public interface MechanismSim extends AutoCloseable {
   ...
   *
   * <p>Called by {@link PumpkinSim#tick()}. In order: read the volts the device is commanding at the
   * present bus voltage, clamp them to that bus, step the plant, then write the resulting
   * <b>rotor</b> position and velocity back through the {@link org.pumpkinlib.core.spi.SimMotorHandle}
   ...
   *     {@link org.pumpkinlib.core.compat.Clock#dt()}
   */
  ...
  /**
   * The raw WPILib model, for the one-in-twenty thing PumpkinLib does not model.
   *
   * <p>Exactly one of {@code ElevatorSim}, {@code SingleJointedArmSim}, {@code FlywheelSim} or
   * {@code DCMotorSim} for a plant built by {@link PumpkinSim}; whatever a hand-written
   ...
   *
   * <p>Called by {@link PumpkinSim#tick()} before {@link #update(double)}, with the voltage computed
   ...
   *
   * <p>This is the number {@link PumpkinSim#stressTest()} sums and prints at boot. It comes from the
