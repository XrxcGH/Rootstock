 ...
 * every duration PumpkinLib measures reads one clock, and a headless clock that did not go through
 ...
 * It is the same order {@code PumpkinTest.stepSeconds} documents.
 *
 * <h2>Usage</h2>
 *
 * <pre>{@code
 * try (HeadlessClock clock = HeadlessClock.install()) {
 *   clock.driving(elevator::periodic, PumpkinSim::tick);
...
public final class HeadlessClock implements AutoCloseable {
   ...
   * here are the loop body, and {@link PumpkinSim#tick()} is usually one of them.
   *
