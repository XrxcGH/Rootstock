 ...
 * PumpkinLib will not make a team's build depend on that. So the adapter is a separate artifact
 * ({@code dev.pumpkinlib:pumpkinlib-maplesim}) discovered by {@code ServiceLoader}, this interface is
 * the contract it implements, and <b>no PumpkinLib build file names maple-sim</b>. A team can never
 * fail to build because of it. See {@link PumpkinFieldSim}.
 *
 * <h2>The degraded path is a supported configuration, not a failure path</h2>
 *
 * <p>Roadmap risk R13. {@link PumpkinFieldSim#field()} always returns something: the adapter when it is
 * installed, and {@link PumpkinFieldSim#kinematic()} when it is not. The kinematic world integrates
...
public interface FieldSim {
   ...
   *
   * <p>Called <b>only</b> from {@link PumpkinSim#tick()}, which returns immediately unless
   * {@link org.pumpkinlib.core.compat.Platform#isSimulation()}. maple-sim's own documentation warns
