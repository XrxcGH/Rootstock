...
final class WpilibPlantSim implements MechanismSim {
   ...
   * {@code org.pumpkinlib.hardware.sim.SimMotorIO.kDefaultSpinUpSeconds}, so a mechanism with no
  ...
  private double m_busVolts = PumpkinSim.kNominalBusVolts;
  ...
  public void update(double dtSeconds) {
    ...
    double bus = Double.isFinite(m_busVolts) && m_busVolts > 0.0 ? m_busVolts : PumpkinSim.kNominalBusVolts;
    ...
    // which has two consequences PumpkinLib does not want. First, it reads the HAL, so a plant
    // stepped through it cannot run on a machine with no WPILib natives — which is CI, and which is
    // the property PumpkinSim.stepPlants() exists to provide. Second, it clamps to whatever was last
    // pushed into RoboRioSim rather than to the bus voltage PumpkinSim just computed, so a team
   ...
   * {@code org.pumpkinlib.hardware.sim.SimMotorIO.inertiaFor}: a mechanism must not change its
