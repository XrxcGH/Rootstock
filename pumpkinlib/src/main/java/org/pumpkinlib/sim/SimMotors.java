 ...
 * references, so that a REV-only team installing {@code dev.pumpkinlib:pumpkinlib} never receives
 * Phoenix code. The real, first-class path is therefore not this class at all — it is
 * {@link MotorIO#simHandle()}, implemented inside {@code pumpkinlib-phoenix6} and
 * {@code pumpkinlib-revlib} with ordinary imports, where the vendor's method names are checked by the
 ...
 * {@link PumpkinException} naming the runtime type, because the alternative — returning a handle that
...
public final class SimMotors {
   ...
   *       {@code SparkFlexSim};
   *   <li>anything else — a thrown {@link PumpkinException} naming the type.
   * </ol>
   *
   ...
   * @return the handle
   * @throws PumpkinException if the type is not supported, or if a supported type is missing a method
        ...
        .orElseThrow(
            () ->
                PumpkinException.of(
                    ...
                    "a SimMotorHandle, a PumpkinLib MotorIO, a Phoenix 6 device (anything with "
                    ...
                    "let the mechanism build its MotorIO through MotorIOFactory and pass that — "
                        ...
                        + "PumpkinSim.dumpDevices() to find its SimDevice key and use "
   ...
   * {@link PumpkinSim#dumpDevices()} prints the exact strings this method wants.
   *
   ...
   *     volts
   * @return the handle
   * @throws PumpkinException if no such device is registered
   */
  public static SimMotorHandle ofSimDevice(
    ...
    if (device.getNativeHandle() <= 0) {
      throw PumpkinException.of(
          ...
          '"' + deviceKey + '"',
          "a key printed by PumpkinSim.dumpDevices()",
          "construct the device before calling this, then run PumpkinSim.dumpDevices() and copy the "
  ...
  public static String describe(Object motorController) {
    ...
    if (motorController instanceof MotorIO) {
      return type + " (PumpkinLib MotorIO — the compiled, vendor-checked seam)";
    }
   ...
   * {@code pumpkinlib-phoenix6} calls with real imports: {@code setSupplyVoltage(double)},
   ...
   * is, after {@code SparkBaseConfig}'s conversion factors, which for a PumpkinLib-configured SPARK
   ...
   * inference is roadmap risk R3 and it is pinned by {@code pumpkinlib-revlib}'s own test; this class
  ...
  private static Method require(Class<?> type, String name, Class<?>... parameters) {
    ...
    throw PumpkinException.of(
        ...
        "the vendor library on the classpath is a version PumpkinLib has not been checked against. "
  ...
  private static Object invoke(Method method, Object target, String what, Object... arguments) {
    try {
      ...
      throw new PumpkinException(
          "PumpkinLib: SimMotors could not call "
              + what
              + " on "
