  ...
  private void endSweep(boolean interrupted) {
    ...
    m_lastLogPath.ifPresent(
        path ->
            Alerts.info(
                kAlertGroup,
                "AdvantageKit logs are not directly loadable by the SysId GUI. PumpkinLib wrote a "
    ...
    try {
      ...
      Alerts.warning(
          ...
          "Could not open a fresh WPILog for the "
              ...
              + " sweep ("
              + e.getMessage()
              + "). The sweep will still run and PumpkinLib will still fit the gains on the robot; "
  ...
  private static Subsystem placeholderSubsystem() {
    return new Subsystem() {
      ...
      public String getName() {
        return "PumpkinTuningPlaceholder";
      }
    };
  }
