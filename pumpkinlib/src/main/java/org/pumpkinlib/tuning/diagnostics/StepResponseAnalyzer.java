...
public final class StepResponseAnalyzer {
  ...
  private static String recommendation(
    ...
    return switch (classification) {
      ...
      case SLUGGISH ->
          "Turn kP up. PumpkinLib will multiply it by 1.6 and try again - accept, or set it "
              + "yourself.";
      case OVERSHOOT_RING ->
          "Add damping: cut kP by 30% or add kD. PumpkinLib will try kD first, because that keeps "
              + "the mechanism fast.";
      case OSCILLATING ->
          oscillationHz >= kNoiseFrequencyHz
