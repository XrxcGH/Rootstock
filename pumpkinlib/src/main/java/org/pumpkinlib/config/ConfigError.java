 ...
 * every config stores its own list, {@code PumpkinRegistry.addAll(...)} gathers all of them plus the
...
public record ConfigError(
  ...
  /**
   * One line, for an alert title, a checklist row or the {@code /Pumpkin/Config/Errors} array.
   *
  ...
  public String describe() {
    ...
    sb.append("org.pumpkinlib.config.ConfigError [")
        .append(severity)
        .append("]: PumpkinLib config error in \"")
        .append(owner)
        .append('"')
        .append(nl)
        .append(nl);
  ...
  /**
   * This error as the core-tier {@link SafeMode.Fault} that {@code PumpkinRegistry} understands.
   *
   * <p>Field for field, with no information lost: {@code SafeMode.Fault} was given exactly this
   * shape so that {@code org.pumpkinlib.core} never has to name a config type, and so this adapter
  ...
  /**
   * Bulk conversion for {@code PumpkinRegistry}'s fault extractor.
   *
