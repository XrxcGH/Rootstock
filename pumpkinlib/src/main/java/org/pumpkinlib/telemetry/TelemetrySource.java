 ...
 * {@code PumpkinRegistry.addAll(...)} (D27), and never again. There is no per-cycle callback into
 * this interface and there is no {@code TelemetrySink}: values are <b>pushed</b> by the implementer
 * through the {@link PumpkinLog} statics during its own {@code periodic()}.
 *
 ...
 * {@code PumpkinLog} "the single static facade every domain calls", and a facade every domain calls is
 * a push facade.
 *
...
public interface TelemetrySource {
  ...
  /**
   * A stable, unique, path-safe name. Becomes the log key segment: {@code "Pumpkin/<name>/..."}.
   *
