 ...
 * reason in a build is collected into {@code build/pumpkin/replay-exemptions.txt} as a CI artifact so
 * a mentor can read the whole list in one place.
 *
 * <p>PumpkinLib's own sanctioned exemptions are the two the design names, and there are no others:
 *
 * <ul>
 *   <li>{@code PumpkinTracer} reads the un-injected hardware clock, because it must report real wall
