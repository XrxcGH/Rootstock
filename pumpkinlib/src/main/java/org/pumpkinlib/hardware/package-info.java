 ...
 * is worth.
 *
 * <p>So {@link org.pumpkinlib.hardware.MotorIO} takes {@code setPositionGoal(outputRotations,
 ...
 *
 * <ul>
 *   <li>{@link org.pumpkinlib.hardware.MotorIO} — the seam. Read its class javadoc first; the unit
 *       contract in it is the whole design.
 *   <li>{@link org.pumpkinlib.hardware.MotorInputs} — everything read back, hand-written
 *       {@code LoggableInputs}, with NaN meaning "not measured" and never a frozen zero.
 *   <li>{@link org.pumpkinlib.hardware.MotorCapabilities} — what a backend can actually do, so a
 *       caller asks instead of discovering by failure, and so a {@code ControlLocation} downgrade is
 *       loud and printed.
 *   <li>{@link org.pumpkinlib.hardware.SignalSet} — subscription and read as one declaration, so
 *       Phoenix's one-transaction CAN efficiency survives the abstraction and an unsubscribed field
 *       cannot be silently frozen.
 *   <li>{@link org.pumpkinlib.hardware.RioControlLoop} — the single implementation of
 *       {@code RIO_FULL}, shared by every backend that needs it.
 *   <li>{@link org.pumpkinlib.hardware.MotorIOFactory} — backend selection in one place, with
 *       vendors registering themselves rather than being named by core.
 *   <li>{@link org.pumpkinlib.hardware.AbsoluteEncoderIO}, {@link org.pumpkinlib.hardware.GyroIO},
 *       {@link org.pumpkinlib.hardware.DigitalSensorIO} — the three other sensor seams core owns.
 * </ul>
 *
