 ...
 *       AlertRegistry#poll()} — the same two calls {@code PumpkinRobot.robotPeriodic()} makes;
 *   <li>{@link Clock#tick()}, so the library's cycle counter advances exactly once per loop, from
 *       0, the way {@code PumpkinLifecycle.beforeUserPeriodic()} advances it on the robot;
 ...
 * in-memory {@code LogTable} through the {@code PumpkinLog} facade, and neither the facade nor the
 * governor exists yet. {@code org.pumpkinlib.test} is <em>not</em> on ArchUnit rule 1c's allowlist,
 * so when they land they must go through {@code PumpkinLog} rather than naming {@code Logger} here.
...
public final class PumpkinTest implements BeforeEachCallback, AfterEachCallback {
   ...
   *
   * @return the extension. Register it with {@code @RegisterExtension final PumpkinTest t =
   *     PumpkinTest.headless();}
   */
  public static PumpkinTest headless() {
    return new PumpkinTest();
  }
  ...
  public void beforeEach(ExtensionContext context) {
    ...
    if (!halUp) {
      throw new IllegalStateException(
          "PumpkinTest.headless(): HAL.initialize("
              + kHalTimeoutMillis
  ...
  /**
   * Every alert currently raised through {@code org.pumpkinlib.core.alert}, in registration order.
   *
   * <p>Named for the alert facade, not the deleted {@code PumpkinFaults}. A test asserting {@code
  ...
  public List<String> raisedAlerts() {
    ...
    for (PumpkinAlert alert : AlertRegistry.active()) {
  ...
  /**
   * The wall time the most recent loop took, in milliseconds, as measured by {@link PumpkinTracer}.
   *
   * @return the last loop's duration in milliseconds, 0 if no loop has run
   */
  public double lastLoopMillis() {
    return PumpkinTracer.loopMillis();
  }
  ...
  private void requireStarted() {
    if (!m_started) {
      throw new IllegalStateException(
          "PumpkinTest was used outside a test. Register it with "
              + "@RegisterExtension final PumpkinTest t = PumpkinTest.headless(); - a plain field "
