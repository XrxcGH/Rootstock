...
/**
 * How PumpkinLib configures AdvantageKit's {@code Logger}, as an immutable value.
 *
 ...
 * mutate the object after {@code PumpkinLifecycle} has read it, which was never a feature.
 *
 * <p>Consumed by {@code PumpkinLifecycle.create(LogConfig)} and by
 * {@code PumpkinRobot(LogConfig)}.
 */
public final class LogConfig {
  ...
  /**
   * From scratch: {@code PumpkinLifecycle} CONFIGURES AdvantageKit's {@code Logger} and STARTS it.
   *
   * <p>What a new robot project, and {@code PumpkinTemplate}, want.
   *
   ...
   *
   * <p>{@code PumpkinLifecycle} configures nothing that would restart it and never calls
   * {@code start()} a second time. This is the one a team adding {@code PumpkinLifecycle} to an
   ...
   * PumpkinLifecycle would otherwise install are simply not acted on.
   *
   ...
   * {@code PumpkinLifecycle} reads this to decide whether to call {@code Logger.start()}.
   *
   ...
   * {@code PumpkinLifecycle}, not by user code.
   *
   * @return the mode
   */
  ...
  /**
   * Whether the {@code /Pumpkin/Driver/**} NT mirror is published. Default true.
   *
