 ...
 * {@code org.pumpkinlib.core} ever naming them.
 *
 * <p><b>Why this interface exists at all.</b> ArchUnit rule 9 says every dependency arrow points into
 * core. {@code PumpkinLifecycle} must call {@code PumpkinLog.beforeUserPeriodic()} and
 ...
 * {@code PumpkinLifecycle.create()}, in a hand-written priority order. Out-of-jar hooks (the vendor
 * adapters, and {@link VisionSimHook}) keep {@code ServiceLoader}, because that is where it is
 * load-bearing. {@code PumpkinLifecycle.hooks()} returns the merged, priority-ordered view of both,
 ...
 * robot down for a telemetry problem, and {@code PumpkinLifecycle} wraps each call in a
...
public interface LifecycleHook {
  ...
  /**
   * One-time initialization, run from {@code PumpkinLifecycle.init()} after every registrant has been
   ...
   *
   * <p>{@code PumpkinSim} is the implementor: it steps every declared {@link MechanismGeometry}'s
   ...
   *
   * <p>Called from {@code PumpkinLifecycle.close()}. Must not throw.
   */
  default void close() {}
}
