 ...
 *
 * <p>Drop-in shape-compatible with {@code edu.wpi.first.wpilibj.Timer} for the methods PumpkinLib
 * uses, but every read is {@link Clock#seconds()}. <b>PumpkinLib code never constructs a
...
public final class PumpkinStopwatch {
   ...
   */
  public static PumpkinStopwatch started() {
    PumpkinStopwatch sw = new PumpkinStopwatch();
    sw.start();
    return sw;
  }
  ...
  public String toString() {
    return String.format("PumpkinStopwatch[%.3f s, %s]", get(), m_running ? "running" : "stopped");
  }
}
