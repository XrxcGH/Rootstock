...
/**
 * THE single slice scheduler: one shared round-robin for everything in PumpkinLib that is periodic
 * but not every-loop.
 *
...
public final class SliceScheduler {
  ...
  public static final String kSweepCyclesTopic = "/Pumpkin/Health/SweepCycles";

  /** NetworkTables topic carrying the name of the slice that ran on the most recent loop. */
  public static final String kLastSliceTopic = "/Pumpkin/Health/LastSlice";

  /** Alert group for the scheduler's own problems. */
  private static final String kGroup = "PumpkinLib";
  ...
  private static PumpkinAlert m_failureAlert;
   ...
   *     before the first {@code /} selects the {@link PumpkinTracer} budget the slice is timed
   ...
   * budgeted with {@link PumpkinTracer}. If you are not sure, you want {@link #register}.
   *
   ...
   * by {@code PumpkinRobot}, which is how you end up with two schedulers.
   *
   ...
   *
   * <p>{@code PumpkinRobot} calls this exactly once per {@code robotPeriodic()}. Nothing else calls
   * it.
   *
   * <p>Declared {@code public} rather than package-private as sketched in the design: the one caller
   * lives in {@code org.pumpkinlib.core}, a different package, so package-private would not compile.
  ...
  private static void runGuarded(String name, Runnable work) {
    ...
    PumpkinTracer.Scope scope = PumpkinTracer.section(section);
    try {
      work.run();
      scope.close();
      ...
      if (m_failureAlert == null) {
        m_failureAlert =
            Alerts.warning(
                kGroup,
                "a health slice threw - see /Pumpkin/Health/LastSlice",
