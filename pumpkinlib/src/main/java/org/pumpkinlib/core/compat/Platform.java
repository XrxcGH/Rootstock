 ...
 *
 * <p>Nothing else in PumpkinLib may call {@code RobotBase.isSimulation()},
 * {@code RobotController.*} or {@code Filesystem.*} — ArchUnit rule 2 confines all three to this
 * package and to {@code org.pumpkinlib.field}. That is what makes the 2027 rename an import rewrite
 ...
 * {@code org.pumpkinlib.core.match.MatchContext}. Callers say {@code MatchContext.isDiagnostics()}.
 ...
 * allowlist entry.
 *
 * <p><b>{@code PumpkinField} is NOT in this package.</b> D14 gives the one {@code PumpkinField} to
 * {@code org.pumpkinlib.field}, which joins the same ArchUnit-fenced tier because it is the file
...
public final class Platform {
   ...
   *
   * <p>This is where {@code pumpkin/robot-id}, {@code pumpkin/target-state.json} and the persistent
   ...
   *
   * <p>Holds {@code pumpkin/disabled.txt} (the runtime kill switch), {@code pumpkin/gains.json}, the
  ...
  // org.pumpkinlib.core.health.builtin without naming RobotController, which rule 2 forbids outside
  // this package.
  // -------------------------------------------------------------------------------------------
   ...
   * and it is the <i>only</i> HAL type PumpkinLib would otherwise touch. Flattening it here means the
