 ...
 *
 * <p>Holds every {@link PumpkinAlert} created through {@link Alerts}, evaluates the self-driving
...
public final class AlertRegistry {
  ...
  /**
   * The driver-mirror display cap: {@code /Pumpkin/Driver/Blocking} shows at most this many rows,
   * with {@code /Pumpkin/Driver/BlockingMore} carrying the rollup for the rest.
   */
  ...
  private static final String kSelfGroup = "PumpkinLib";

  private static final List<PumpkinAlert> m_alerts = new CopyOnWriteArrayList<>();
  private static final AtomicInteger m_nextIndex = new AtomicInteger();
  private static PumpkinAlert m_budgetAlert;
   ...
   */
  public static void register(PumpkinAlert alert) {
   ...
   */
  public static List<PumpkinAlert> all() {
  ...
  public static List<PumpkinAlert> active() {
    return m_alerts.stream().filter(PumpkinAlert::isActive).collect(Collectors.toUnmodifiableList());
  }
  ...
  public static Optional<Severity> worst() {
    return m_alerts.stream()
        .filter(PumpkinAlert::isActive)
        .map(PumpkinAlert::severity)
  ...
  public static List<PumpkinAlert> blocking() {
    return m_alerts.stream()
        ...
        .sorted(
            Comparator.comparing(PumpkinAlert::severity)
                .reversed()
                .thenComparingInt(PumpkinAlert::registrationIndex))
  ...
  /**
   * The at-most-{@value #kDriverDisplayCap} rows for {@code /Pumpkin/Driver/Blocking}.
   *
  ...
  public static List<String> driverBlockingRows() {
    return blocking().stream()
        ...
        .map(PumpkinAlert::driverLine)
        .collect(Collectors.toUnmodifiableList());
  }

  /**
   * The rollup row for {@code /Pumpkin/Driver/BlockingMore}.
   *
  ...
  public static Optional<String> budgetDiagnosis() {
    List<PumpkinAlert> blocking = blocking();
    if (blocking.size() <= kBlockingBudget) {
      return Optional.empty();
    }
    String offenders =
        blocking.stream().map(PumpkinAlert::describe).collect(Collectors.joining("\n  "));
    return Optional.of(
   ...
   *
   * <p>Registered as one slice of the shared health rotation by {@code PumpkinRobot} — it must not
   * be called every loop, and it must not install its own rate gate.
   */
  public static void poll() {
    for (PumpkinAlert alert : m_alerts) {
      alert.poll();
    }
    updateBudgetAlert();
  }
  ...
  /**
   * The rollup published to {@code /Pumpkin/Health/Summary}. Returned as a value rather than
   ...
   * {@code /Pumpkin/Health/Summary/{Worst, ActiveCount, BlockingCount, MatchReady}}.
   *
  ...
  public static String describe() {
    List<PumpkinAlert> active = active();
    if (active.isEmpty()) {
      return "PumpkinLib alerts: none active (" + m_alerts.size() + " registered)";
    }
    List<PumpkinAlert> ordered = new ArrayList<>(blocking());
    active.stream().filter(a -> a.impact() != MatchImpact.BLOCKS_MATCH).forEach(ordered::add);
    return "PumpkinLib alerts: "
        + summary().describe()
        + "\n  "
        + ordered.stream().map(PumpkinAlert::describe).collect(Collectors.joining("\n  "));
  }
  ...
  public static void resetForTest() {
    for (PumpkinAlert alert : List.copyOf(m_alerts)) {
      alert.close();
    }
    m_alerts.clear();
    m_nextIndex.set(0);
  ...
  static PumpkinAlert create(
      String group, String text, Severity severity, MatchImpact impact, BooleanSupplier condition) {
    PumpkinAlert alert =
        new PumpkinAlert(group, text, severity, impact, condition, m_nextIndex.getAndIncrement());
    m_alerts.add(alert);
    return alert;
  }

  /** Removes a closed alert. Called by {@link PumpkinAlert#close()}. */
  static void unregister(PumpkinAlert alert) {
  ...
  private static void updateBudgetAlert() {
    ...
    if (!m_budgetReported) {
      ...
      System.out.println("PumpkinLib: " + diagnosis.get());
    }
  }
}
