...
import org.pumpkinlib.config.VelocityConfig;
...
public final class VelocityMechanism extends Mechanism {
  ...
  private void applyClosedLoop(double dt) {
    ...
    if (m_location.profileOnRio() && Double.isFinite(accelLimit) && accelLimit > 0.0) {
      ...
      m_setpointUser = m_setpointUser + PumpkinMath.clamp(m_goal - m_setpointUser, -step, step);
    } else {
  ...
  private static Gains resolveGains(
      Gains declared, PlantPrior prior, PumpkinAlert derived, String name) {
    if (declared == null) {
    ...
    derived
        .text(
            name
                ...
                + "running in simulation, so PumpkinLib derived "
