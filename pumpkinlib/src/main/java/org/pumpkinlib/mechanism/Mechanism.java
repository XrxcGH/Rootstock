...
import org.pumpkinlib.core.selftest.SelfTestable;
...
/**
 * The base every PumpkinLib mechanism extends — and one of exactly two abstract classes the
 ...
 * what makes {@code PumpkinRegistry.addAll(m_elevator, m_arm, m_intake)} the <i>only</i> registration
 ...
 *
 * <p>CORE <i>declares</i> a {@link MechanismGeometry}; {@code PumpkinSim} owns plant construction and
...
public abstract class Mechanism
  ...
  /**
   * The absolute log prefix, {@code "Pumpkin/<name>"}, precomputed once.
   *
   ...
   * feature PumpkinLib does not model.
   *
   ...
   * {@code PumpkinSim} owns and steps the plant. See {@link #simulationPeriodic()}.
   *
  ...
  public final void periodic() {
    try {
      ...
      PumpkinLog.processInputs(schema().inputsKey(), m_inputs);
      onPeriodic();
   ...
   * MechanismGeometry}, the backend exposes {@code MotorIO.simHandle()}, and {@code PumpkinSim} owns
   * the plant and steps it. That split is what makes the same mechanism code run identically in real,
   * sim and replay, and what keeps {@code org.pumpkinlib.core} free of any dependency on the
   * simulation domain.
   *
   * <p>Overriding it to build a plant would put a second, unlogged integrator next to the one {@code
   * PumpkinSim} steps, and the two would disagree.
   */
  @Override
  public final void simulationPeriodic() {
    // Intentionally empty. See the javadoc: PumpkinSim owns plant construction and the tick.
  }

  /**
   * The staged writer for this mechanism's {@code Pumpkin/<Name>/} block.
   *
   ...
   * is not enforced here: {@code PumpkinRegistry.addAll} catches it in the same collected-not-thrown
