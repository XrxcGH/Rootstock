 ...
 * why {@code MatchContext} exists at all. Rule 8 is what lets {@code org.rootstock.pure} be unit
 ...
 * packages ({@code org.rootstock.tuning}, {@code org.rootstock.mechanism}, {@code
 * org.rootstock.hardware}) that do not exist in M1. They are written now, marked {@code
...
public final class ArchitectureTest {
  ...
  /** The eight domain packages rule 9 forbids {@code org.rootstock.core..} from naming. */
  private static final List<String> kForbiddenFromCore =
      List.of(
          "org.rootstock.telemetry",
          "org.rootstock.tuning",
          "org.rootstock.sim",
          "org.rootstock.vision",
          "org.rootstock.drive",
          "org.rootstock.auto",
   ...
   * to {@code :rootstock}, and {@code :rootstock-phoenix6} / {@code :rootstock-revlib} depend on
   * <em>it</em>, never the other way round. So the classes ArchUnit imports here are the CORE
   * artifact's and nothing else, and "nowhere in {@code org.rootstock}" is exactly "nowhere in the
  ...
  static void rule1a_theAnalysedSetIsTheCoreArtifactAndNamesNoVendor(JavaClasses classes) {
    for (String required :
        List.of(
            ...
            "org.rootstock.hardware.sim.SimMotorIO",
            "org.rootstock.units.MechanismUnits")) {
    ...
    for (JavaClass clazz : classes) {
      if (clazz.getPackageName().startsWith("org.rootstock.hardware.phoenix")
          || clazz.getPackageName().startsWith("org.rootstock.hardware.rev")) {
    ...
    if (!adapterLeaks.isEmpty()) {
      throw new AssertionError(
          "the core artifact must not contain any vendor adapter class. A REV-only team installs "
              + "dev.rootstock:rootstock and must never receive Phoenix code, which is precisely "
   ...
   * {@code org.rootstock.telemetry} (the {@code RootstockLog} facade), {@code org.rootstock.core}
   * (the {@code RootstockRobot} / {@code RootstockLifecycle} root — the package itself, not its
   * subpackages), {@code org.rootstock.tuning} and {@code org.rootstock.viz}.
   *
   * <p>Note the allowlist entry {@code "org.rootstock.core"} has no trailing {@code ..}: {@code
      ...
      classes()
          ...
          .because(
              "rule 1c bounds the blast radius of the R18 fork contingency to four packages. "
                  + "Every other package publishes through RootstockLog and never touches Logger.")
   ...
   * org.rootstock.hardware} landed. The package-only spelling was an accident of M1, where the only
   * inputs classes that existed were hypothetical: the seam's four inputs types live beside their IO
   * interfaces in {@code org.rootstock.hardware}, not in a package called {@code io}, and moving
   ...
   *
   * <p>Exactly three packages may name these five types: {@code org.rootstock.core.compat} (the
   * facades), {@code org.rootstock.field} (D14 — it absorbs the 2027 field-origin move, and it is
   * owned by the drive/auto domain but lives inside this same fence) and {@code
   * org.rootstock.core.match} ({@code MatchContext}, the single {@code DriverStation} reader).
   *
  ...
  static final ArchRule rule2_volatileWpiApiIsConfined =
      classes()
          .that()
          .resideOutsideOfPackages(
              "org.rootstock.core.compat..", "org.rootstock.field..", "org.rootstock.core.match..")
          .should()
          .onlyDependOnClassesThat()
          .haveNameNotMatching(
   ...
   * RootstockStopwatch} is the Clock-backed replacement and has the same shape.
   */
  @ArchTest
      ...
      noClasses()
          ...
          .because(
              "an instance Timer is a wall clock in disguise: it neither replays nor steps under "
                  + "SimHooks. Use RootstockStopwatch, which is Clock-backed and shape-compatible.");
   ...
   * schema; both are removed or effectively dead on the 2027 line. Everything Rootstock publishes
   * goes through NT4 under {@code /Rootstock/}, which is the contract the shipped Elastic and
  ...
  static final ArchRule rule4_noDeadDashboards =
      noClasses()
          ...
          .because(
              "the /Rootstock/ NT4 namespace is a contract the shipped dashboard layouts bind to. "
   ...
   *
   * <p>No Rootstock type overrides {@code IterativeRobotBase.robotInit()} and no Rootstock public
   * API declares a method named {@code robotInit}. {@code RootstockRobot} initialises in its
   * constructor (D13a); {@code RootstockLifecycle}'s method is {@code init()} (D29).
   *
      ...
      noMethods()
          ...
          .because(
              "constructor initialisation is WPILib's recommended shape and the reason robotInit() "
                  + "was deprecated. RootstockRobot initialises in its constructor (D13a) and "
                  + "RootstockLifecycle's hook is init() (D29).");
   ...
   *
   * <p><strong>Vacuous in M1</strong> — {@code org.rootstock.tuning} does not exist yet. Written
  ...
  static final ArchRule rule7_noUserExtendableAbstractClasses =
      noClasses()
          ...
          .and()
          .haveNameNotMatching(".*\\.RootstockAutoMode(\\$.*)?")
   ...
   * v3. Rootstock's mechanisms implement the {@code Subsystem} <em>interface</em>; nothing in the
  ...
  /**
   * RULE 8. {@code org.rootstock.pure} imports nothing from {@code edu.wpi.first}.
   *
   ...
   * this test source set runs in CI. It is also why {@code RootstockMath.deadband2d} returns the
  ...
  static final ArchRule rule8_pureIsHalFree =
      noClasses()
          .that()
          .resideInAPackage("org.rootstock.pure..")
          .should()
          .dependOnClassesThat()
  ...
  /**
   * RULE 9, measured per 9a and 9b. No type in {@code org.rootstock.core..} <em>declares</em> a
   ...
   * {@code RobotMode} and {@code Tier} into {@code org.rootstock.core.spi} (D31, D33) rather than
   ...
   * RootstockRegistry.addAll(Object...)} routes registrants by {@code instanceof}, which crosses in
   ...
   * already works.
   *
   * <p>{@code org.rootstock.core.spi} is the one downward edge, and it points <em>into</em> core:
  ...
  static final ArchRule rule9_coreDeclaresNothingFromOuterDomains =
      classes()
          .that()
          .resideInAPackage("org.rootstock.core..")
   ...
   * members().that().areDeclaredInClassesThat().resideInAPackage("org.rootstock.core..")}.
   *
  ...
  /**
   * RULE 10. Only {@code org.rootstock.core.match..} may name {@code
  ...
  static final ArchRule rule10_onlyMatchReadsDriverStation =
      classes()
          .that()
          .resideOutsideOfPackages("org.rootstock.core.match..")
          .should()
          .onlyDependOnClassesThat()
  ...
  /**
   * RULE 11. No explicit {@code throw} in {@code org.rootstock.mechanism..} or {@code
   * org.rootstock.hardware..} outside constructors, static factories and {@code Validation}.
   *
  ...
  static final ArchRule rule11_mechanismsDoNotThrowOutsideConstruction =
      noMethods()
          .that()
          .areDeclaredInClassesThat()
          .resideInAnyPackage("org.rootstock.mechanism..", "org.rootstock.hardware..")
          .and()
          .areNotStatic()
          .and()
          .areDeclaredInClassesThat()
