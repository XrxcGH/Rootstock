...
public sealed interface FeedbackSpec
   ...
   *
   * <p>True only for {@link FusedCancoder}. When true, Rootstock must do nothing in Java — the
  ...
  record FusedCancoder(
    ...
    public String describe() {
      return sensorDescription()
          ...
          + ". Phoenix fuses on-device, so Rootstock does NOT re-seed in Java.";
    }
  }
  ...
  record SparkAbsolute(Angle zeroOffset, boolean inverted) implements FeedbackSpec {
    ...
    public String describe() {
      return sensorDescription()
          ...
          + (inverted ? ", inverted" : "")
          + ". Rootstock re-seeds the rotor from it at boot and before each move.";
    }
  }
