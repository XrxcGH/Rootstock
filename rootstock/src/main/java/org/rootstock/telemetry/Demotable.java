...
// Demotable STAYS in org.rootstock.telemetry and must not follow LogConfig/RobotMode/Tier into
// core.spi (D33). It is never named in an org.rootstock.core or org.rootstock.core.spi signature —
// RootstockLog.processInputs has no Demotable overload, LogConfig has no demotable field, and the
// parameter appears only on RootstockLog.critical(...)/log(...) inside this package. ArchUnit rule 9 is
 ...
 *
 * <p><b>Orthogonal to {@code org.rootstock.core.spi.Tier}.</b> Tier answers <i>"may this key
 ...
 *
 * <p>A demoted key is published every {@value RootstockBudget#kDemotedPublishEveryN}th cycle instead of
 * every cycle. It is never deleted, never silently retyped, and every demotion bumps
 * {@code Rootstock/Log/SchemaGeneration} so replay and the triage tooling can see the boundary.
 *
 ...
 *
 * <ul>
 *   <li><b>Type level.</b> {@link RootstockLog#processInputs(String,
 ...
 *   <li><b>Runtime.</b> {@link RootstockBudget#isDemotionEligible(String)} refuses any key whose path
 *       contains {@code "/Inputs/"} or lies under {@code Rootstock/Driver/}, and marking one raises a
 *       named alert instead of quietly succeeding.
 *   <li><b>Build level.</b> {@code rootstock-lint} fails the build on the same two patterns.
 * </ul>
 *
...
public enum Demotable {
  ...
  /**
   * The governor may publish this key every {@value RootstockBudget#kDemotedPublishEveryN}th cycle when
   * it has been over budget for {@value RootstockBudget#kOverBudgetCycles} consecutive cycles.
   *
   * <p>Legal only on <b>outputs</b>. Passing it for a key containing {@code "/Inputs/"} or under
   * {@code Rootstock/Driver/} is refused at runtime and is a build error under {@code rootstock-lint}.
   */
  YES
}
