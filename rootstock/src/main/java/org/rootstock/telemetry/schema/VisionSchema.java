...
/**
 * The section 3.3 key block — {@code Rootstock/Vision/&lt;Camera&gt;/} — as both a published contract
 * and the writer that fills it.
 *
 * <p>One instance per camera, created once and reused; every absolute key is concatenated in the
 * constructor so the per-cycle path allocates nothing. The roll-up keys at {@code Rootstock/Vision/}
...
public final class VisionSchema {
  ...
  public static final String kPrefix = RootstockLog.kRoot + "/Vision";
   ...
   *
   * @return e.g. {@code "Rootstock/Vision/FrontLeft"}
   */
  ...
  public void connected(boolean connected) {
    RootstockLog.critical(m_keyConnected, connected);
  }
  ...
  public void timing(double fps, double latencySeconds) {
    RootstockLog.critical(m_keyFps, fps);
    RootstockLog.critical(m_keyLatencySec, latencySeconds, Units.Seconds);
  }
  ...
  public void robotToCamera(Transform3d robotToCamera) {
    ...
    RootstockLog.log(m_keyRobotToCamera, robotToCamera);
  }
  ...
  public void tags(long[] tagIds) {
    RootstockLog.critical(m_keyTagCount, tagIds.length);
    RootstockLog.critical(m_keyTagIds, tagIds);
  }
  ...
  public void estimatedPose(Pose3d estimatedPose) {
    RootstockLog.critical(m_keyEstimatedPose, estimatedPose);
  }
  ...
  public void verdict(boolean accepted, String rejectReason) {
    RootstockLog.critical(m_keyAccepted, accepted);
    String reason = accepted || rejectReason == null ? "" : rejectReason;
    RootstockLog.critical(m_keyRejectReason, reason);
  }
  ...
  public void quality(double ambiguity, double avgTagDistanceMeters) {
    RootstockLog.log(m_keyAmbiguity, ambiguity);
    RootstockLog.log(m_keyAvgTagDistanceMeters, avgTagDistanceMeters, Units.Meters);
  }
   ...
   * correct for the array, and {@code RootstockLog}'s array overloads require a unit precisely so that
  ...
  public void stdDevs(double[] stdDevs) {
    RootstockLog.log(m_keyStdDevs, stdDevs, Units.Value, Demotable.YES);
  }
  ...
  public void allTagPoses(Supplier<Pose3d[]> poses) {
    RootstockLog.debugArray(m_keyAllTagPoses, Pose3d.struct, poses);
  }
  ...
  public static void acceptedThisCycle(long count) {
    RootstockLog.critical(kAcceptedThisCycle, count);
  }
  ...
  public static void uptimeFraction(double fraction) {
    RootstockLog.log(kUptimeFraction, fraction);
  }
  ...
  public static void anyCameraOffline(boolean offline) {
    RootstockLog.critical(kAnyCameraOffline, offline);
  }
  ...
  /**
   * Section 3.3's roll-ups at {@code Rootstock/Vision/}.
   *
   * @return the rows
   */
