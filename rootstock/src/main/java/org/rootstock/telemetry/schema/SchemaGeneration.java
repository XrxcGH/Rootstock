...
/**
 * Emits Rootstock's automatic telemetry schema as one deterministic, diffable document.
 *
 ...
 *
 * <h2>Not to be confused with {@code Rootstock/Log/SchemaGeneration}</h2>
 *
 ...
 *
 * <pre>{@code
 * # Rootstock telemetry schema, format 1
 * # key	type	unit	tier	demotable	[# meaning]
 * ## 3.1 Every mechanism - Rootstock/<Name>/ (inputs)
 * Rootstock/<Name>/Inputs/Connected	boolean[] (per motor)	-	CRITICAL	NO
 * }</pre>
 *
...
public final class SchemaGeneration {
  ...
  public static final String kHeader = "# Rootstock telemetry schema, format " + kFormatVersion;
  ...
  public static List<SchemaSection> sections() {
    return List.of(
        new SchemaSection(
            "3.1 Every mechanism - Rootstock/<Name>/ (replayable inputs)", MechanismSchema.inputSchema()),
        new SchemaSection("3.1 Every mechanism - Rootstock/<Name>/ (outputs)", MechanismSchema.outputSchema()),
        new SchemaSection("3.1 Every mechanism - declared extras", MechanismSchema.extraSchema()),
        new SchemaSection("3.2 Drivetrain - Rootstock/Drive/", DriveSchema.schema()),
        new SchemaSection("3.3 Vision - Rootstock/Vision/<Camera>/", VisionSchema.schema()),
        new SchemaSection("3.3 Vision - roll-ups at Rootstock/Vision/", VisionSchema.rollupSchema()),
        new SchemaSection("3.4 Field / ghost contract - Rootstock/Field/", FieldSchema.schema()),
        new SchemaSection("3.5 Alerts and health - Rootstock/Health/", HealthSchema.schema()),
