...
/**
 * Per-section loop-time accounting with declared budgets — the answer to "is Rootstock causing my
 ...
 * ({@code RobotController.getFPGATime()}) on a robot. Every other duration in Rootstock comes from
...
public final class RootstockTracer {
  ...
  private static volatile BreachListener s_breachListener = RootstockTracer::reportBreachToConsole;
   ...
   *
   * <pre>{@code
   * try (var s = RootstockTracer.section("Vision/Consume")) {
   *   consumeFrames();
   * }
   * }</pre>
   *
   ...
   * window, so a section that is never {@code reset()} never breaches. {@code RootstockRobot} calls
   ...
   *
   * <p>This is what a publisher emits as {@code Rootstock/Perf/<section>Ms} every loop. It reads one
  ...
  public static String describe() {
    ...
    if (sections.length == 0) {
      return "RootstockTracer: no sections measured yet.";
    }
    ...
    out.append(
        String.format(
            Locale.ROOT,
            "RootstockTracer: loop %.2f ms (worst %.2f, %d overruns in %d loops, period %.1f ms)%n",
            s_lastLoopMillis,
            s_worstLoopMillis,
            s_overrunCount,
            s_loopCount,
  ...
  public static void setBreachListener(BreachListener listener) {
    s_breachListener = listener == null ? RootstockTracer::reportBreachToConsole : listener;
  }
    ...
    public String message() {
      return String.format(
          ...
          "PERF_%s: %.2f ms against a %.2f ms budget for %d consecutive loops. "
              + "Fix: profile that section, or raise its budget with RootstockTracer.budget(\"%s\", "
  ...
  private static void reportBreachToConsole(Breach breach) {
    ...
    System.err.println("[RootstockTracer] " + breach.message());
  }
  ...
  public static void resetForTest() {
    clear();
    s_breachListener = RootstockTracer::reportBreachToConsole;
  }
  ...
  /**
   * The try-with-resources handle returned by {@link RootstockTracer#section(String)}.
   *
