...
public final class Clock {
  ...
  /**
   * The loop period assumed before {@code RootstockRobot} tells us the real one. Matches WPILib's
   ...
   *
   * <p>Captured once from {@code IterativeRobotBase.getPeriod()} at {@code RootstockRobot}
   ...
   * {@code Rootstock.dt()} god-object accessor. There is deliberately no {@code periodSeconds()} twin:
  ...
  /**
   * Installs the real loop period. Called once, from {@code RootstockRobot}'s constructor, with
   * {@code getPeriod()}.
   *
   * <p>Public because {@code RootstockRobot} is in a different package and because
   * {@code rootstock-testkit} has to be able to say "pretend this robot runs at 100 Hz". It is not
   ...
   * {@code RootstockLifecycle.beforeUserPeriodic()}.
   *
   ...
   * THIS is how every periodic rate in Rootstock is expressed. It is exact under replay, under a
   ...
   *
   * <p>This exists so {@code org.rootstock.sim.HeadlessClock} and {@code RootstockTest} can step time
