 ...
 *
 * <p><b>There is no other converter in Rootstock, and mechanism code never performs a unit
...
public final class MechanismUnits {
   ...
   *
   * <p>Printed at boot, included in the config snapshot, and shown by {@code rootstock doctor}. It
    ...
    if (rigging.isToothed() && teeth > 0 && pitchInches > 0.0) {
      ...
      cont(
          ...
          String.format(
              ...
              percentHigh,
              medium));
      cont(sb, " rides a polygon. Rootstock uses the " + medium.toUpperCase(Locale.ROOT)
          + " ADVANCE.)");
    } else {
      row(
          sb,
  ...
  private void describeRotary(StringBuilder sb, RotaryAxis rotary) {
    ...
    if (rotary.gravity() == org.rootstock.control.GravityMode.COSINE) {
      row(
          sb,
          "horizontal at",
          String.format(
