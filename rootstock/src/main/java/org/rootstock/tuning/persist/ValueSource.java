...
public enum ValueSource {
  ...
  /** Tier 2: {@code src/main/deploy/rootstock/gains.json} — the team's committed, reviewed answer. */
  DEPLOY_FILE,

  /** Tier 3: {@code Platform.persistentDir()/rootstock/gains.json} — the "we tuned it Saturday" file. */
  ROBOT_FILE,
   ...
   *
   * @return e.g. {@code "the committed deploy file (src/main/deploy/rootstock/gains.json)"}
   */
  public String explain() {
    switch (this) {
      ...
      case DEPLOY_FILE:
        return "the committed deploy file (src/main/deploy/rootstock/gains.json)";
      case ROBOT_FILE:
