...
import org.rootstock.core.spi.MechanismGeometrySink;
 ...
 *
 * <p>So Rootstock deletes the block. {@code Mechanism.simulationPeriodic()} does not exist (D18); a
 ...
 * {@code simulationTick} calls {@link #tick()}; {@code RootstockLifecycle} runs that only when
 ...
 * hand. Register the hook with {@code RootstockRegistry.addAll(RootstockSim.hook())} — or call
...
public final class RootstockSim {
  ...
  public static final String kKeyPrefix = "Rootstock/Sim/";
  ...
  /**
   * This hook's priority in {@code RootstockLifecycle}'s ordered list.
   *
  ...
  /** The one sink instance, so {@code RootstockRegistry} can install it and compare it by identity. */
  private static final MechanismGeometrySink s_sink = RootstockSim::declareGeometry;
  ...
  private static RootstockAlert s_brownoutAlert;
  private static RootstockAlert s_unattachedAlert;
  private static RootstockAlert s_declarationAlert;
   ...
   * plant.
   *
   * <p>{@code org.rootstock.core.spi} names {@link MechanismGeometrySink} and does not name this
   ...
   *
   * <p>Hand it to {@code RootstockRegistry.addAll(...)} before creating the {@code RootstockLifecycle}.
   * Its {@code init()} prints the boot report and its {@code simulationTick} is the tick;
   * {@code RootstockLifecycle} only calls the latter in simulation.
   *
  ...
  public static synchronized MechanismSim declare(
    ...
    Objects.requireNonNull(geometry, "RootstockSim.declare: geometry must not be null");
  ...
  public static synchronized void register(String name, MechanismSim sim) {
    Objects.requireNonNull(sim, "RootstockSim.register: sim must not be null");
    String key =
   ...
   * @throws RootstockException if no plant is registered under {@code name}, or if the registered plant
  ...
  public static synchronized void attach(String name, SimMotorHandle handle) {
    ...
    if (!(registration.sim() instanceof WpilibPlantSim plant)) {
      throw RootstockException.of(
          ...
          "a plant built by RootstockSim.declare(...)",
   ...
   * @throws RootstockException if the name is unknown or the controller type is not supported
   */
   ...
   * the year-volatile WPILib types to {@code org.rootstock.core.compat}, and this class is in
   * {@code org.rootstock.sim}. Same call, one hop through the facade.
   *
  ...
  public static synchronized void tick() {
    ...
    RootstockFieldSim.tick(dt);

    RootstockLog.log(kKeyPrefix + "BatteryVolts", volts, Volts);
    RootstockLog.log(kKeyPrefix + "TotalCurrentAmps", s_totalAmps, Amps);
    for (Registration registration : s_sims.values()) {
      MechanismSim sim = registration.sim();
      RootstockLog.log(registration.positionKey(), sim.positionSi());
      RootstockLog.log(registration.velocityKey(), sim.velocitySi());
      RootstockLog.log(registration.currentKey(), sim.currentDrawAmps(), Amps);
      RootstockLog.log(registration.voltsKey(), sim.appliedVolts(), Volts);
    }
   ...
   * bare JVM in CI — the same property ArchUnit rule 8 buys for {@code org.rootstock.pure}, and the
  ...
  public static synchronized void printBootReport() {
    ...
    sb.append("RootstockSim: ").append(s_sims.size()).append(" simulated mechanism(s)\n");
    ...
    RootstockLog.log(kKeyPrefix + "PeakStallAmps", report.peakStallAmps(), Amps);
    RootstockLog.log(kKeyPrefix + "StressTest", report.describe());

    RootstockFieldSim.publishAvailability();
    declarationAlert()
        .text(
            problems.isEmpty()
        ...
        .text(
            unattached.isEmpty()
                ...
                : "SIM_NO_DEVICE: "
                    ...
                    + " will not move in simulateJava. Fix: RootstockSim.attach(name, motor), or let "
  ...
  /**
   * A one-block summary of the simulation's state, for the boot dump and {@code rootstock doctor}.
   *
  ...
  public static synchronized String describe() {
    ...
    sb.append(
        String.format(
            Locale.ROOT,
            "RootstockSim: %d mechanism(s), bus %.2f V, load %.1f A%s%n",
            s_sims.size(),
            s_busVolts,
            s_totalAmps,
    ...
    sb.append(RootstockFieldSim.describe());
  ...
  public static final class ArmBuilder {
    ...
    public MechanismSim build() {
      ...
      if (!Double.isFinite(moi) && m_mass != null && Double.isFinite(length)) {
        ...
        // Rootstock simulates with is the number SingleJointedArmSim was written against.
   ...
   */
  private static RootstockAlert brownoutAlert() {
  ...
  private static RootstockAlert unattachedAlert() {
  ...
  private static RootstockAlert declarationAlert() {
