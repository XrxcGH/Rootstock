 ...
 * {@code org.rootstock.core.spi} <i>declares</i>: {@link org.rootstock.core.spi.MechanismGeometry}
 ...
 * {@link org.rootstock.core.spi.SimMotorHandle} so the plant's state is written back as <b>rotor</b>
 ...
 * {@code siPerOutputRotation()} — the same two numbers {@link org.rootstock.units.MechanismUnits}
 * hands the {@code MotorIO} seam. {@link org.rootstock.sim.RootstockSim} additionally cross-checks the
 ...
 *
 * <ul>
 *   <li>{@link org.rootstock.sim.RootstockSim} — the registry, the tick, the battery, the boot stall
 *       report and the {@code MechanismGeometrySink} the mechanism domain installs.
 *   <li>{@link org.rootstock.sim.MechanismSim} — one plant. {@code raw()} is the escape hatch to the
 *       WPILib model for the one-in-twenty thing Rootstock does not model.
 *   <li>{@link org.rootstock.sim.SimMotors} — turns a vendor motor controller into a
 *       {@code SimMotorHandle} without this package naming a vendor type (ArchUnit rule 1).
 *   <li>{@link org.rootstock.sim.HeadlessClock} — deterministic time, faster than real time, with no
 *       HAL. It installs itself into {@link org.rootstock.core.compat.Clock}'s one time seam rather
 *       than becoming a second time authority (ArchUnit rule 3).
 * </ul>
 *
 * <p><b>maple-sim is discovered, never depended on (design/04 §7.5).</b>
 * {@link org.rootstock.sim.RootstockFieldSim} is a {@code ServiceLoader} lookup for an out-of-jar
 ...
 * {@code core.compat}, so the simulation gate is {@link org.rootstock.core.compat.Platform#isSimulation()}
 * and never {@code RobotBase.isSimulation()}. Rule 3 confines the clock to
 * {@link org.rootstock.core.compat.Clock}. Rule 1c keeps AdvantageKit's {@code Logger} out, so
 * everything published here goes through {@link org.rootstock.telemetry.RootstockLog}.
 */
